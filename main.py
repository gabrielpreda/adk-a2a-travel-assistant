...
async def query_discovery(input) -> None:
    """Test trending topics agent."""
    discovery = await a2a_client.create_task(
        'http://localhost:10022', input
    )
    return discovery
...
async def plan_trip(request: TripRequest):
    """
        Plan trip
    Args:
        request (TripRequest): request with the query
    Returns:
        itinerary (str): Markdown formated text with the itinerary description
    """
    user_input = request.query

    response_text = await query_discovery(user_input)

    logger.info(f"System response: {response_text}")
